# alert on long command
# use by including in zshrc
# source $HOME/dotfiles/bin/zlong_alert.zsh
# requires ogg123 from vorbis-tools package
...
(( ${+zlong_ignore_cmds} )) || zlong_ignore_cmds='vim ssh man less nano emacs emacsclient git'

# Set as true to ignore commands starting with a space
(( ${+zlong_ignorespace} )) || zlong_ignorespace='true'
...
zlong_alert_func() {
    ...
    if [[ "$zlong_internal_send_notifications" != false ]]; then
        ...
        if [[ "$OSTYPE" == "linux-gnu"* ]]; then
            notify-send $message
            # requires ogg123 from vorbis-tools package
            ogg123 -q /usr/share/sounds/freedesktop/stereo/message.oga /usr/share/sounds/freedesktop/stereo/message.oga /usr/share/sounds/freedesktop/stereo/message.oga
